...
Jev（TypeSafeの意味判定API）を、実際の開発作業で安全に使えるかを確かめるためのリポジトリです。現在のプロダクトは、Codex CLIのSkill選択を提案して、その効果と費用を測る **[jevx](jevx/)**（macOS向けRust CLI）です。

## なぜ作ったか

LLMの判定を開発ツールに組み込むとき、「本当に良くなったのか」「失敗したら何が起きるのか」を数字と設計で説明できないまま導入しがちです。このリポジトリでは、Jevに意味の判定だけを任せ、実行や権限はコードが持つ、という線引きで小さく試し、Jevあり／なしを同じ条件で測っています。
