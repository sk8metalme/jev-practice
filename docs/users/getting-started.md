...
同梱の9件の評価用Skillと40ケース（合成30件、匿名化テンプレート10件）を使った2026-09-21時点の記録では、次の差が観測されています。単回結果は40ケースを1回、複数回結果は同じ条件を5回、合計200ケースで測定した歴史的なスナップショットです。
...
現行コードの外部送信なしベースラインは、`cargo run --locked --manifest-path jevx/Cargo.toml -- eval --dry-run --json` を実行して確認してください。APIキーがない場合、Jevモードは`not_run`になります。
