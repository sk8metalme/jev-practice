...
Hook metadataの`trigger` / `source` / `selectedSkill`はwrite前にtrim・許可文字・最大長を検証し、unsafeな値は欠損化する。新規appendはunsafeなidentifierを拒否し、既存schema v1/v2/v3のloadでは該当metadataを正規化・欠損化して分析互換性を保つ。Review receiptも本文を保存せず、task/turn/sessionはhashだけでcostを集計する。
